...
from QuentinBot2 import QuentinBot2
from SamBot import SamBot
from DanBot import DanBot
from MatthewBot import MatthewBot


class ZeroPlayerGame(TwoPlayerGame, QuentinBot, QuentinBot2, SamBot, MatthewBot, DanBot):
    def __init__(self, master, outcome, white_bot, black_bot):
    ...
    def on_release(self, event):
        ...
        if not self.board.is_game_over(claim_draw=True):
            if self.white_bot == "QuentinBot":
                white_bot_move = self.Quentin_AI_move()
            elif self.white_bot == "QuentinBot2":
                white_bot_move = self.Quentin_AI_move2()
            elif self.white_bot == "SamBot":
                white_bot_move = self.Sam_AI_move()
            elif self.white_bot == "DanBot":
                white_bot_move = self.Dan_AI_move()
            elif self.white_bot == "MatthewBot":
                white_bot_move = self.Matthew_AI_move()
            else:
                raise exception('Bot name not recognised')

            self.make_move(white_bot_move)

            split_fen = self.board.fen().split(" ")
            fen = split_fen[0] + split_fen[1] +split_fen[3]
            self.board_store.append(fen)

        # Make sure the AI doesn't play when the game is over
        if not self.board.is_game_over(claim_draw=True):
            if self.black_bot == "QuentinBot":
                black_bot_move = self.Quentin_AI_move()
            elif self.black_bot == "QuentinBot2":
                black_bot_move = self.Quentin_AI_move2()
            elif self.black_bot == "SamBot":
                black_bot_move = self.Sam_AI_move()
            elif self.black_bot == "DanBot":
                black_bot_move = self.Dan_AI_move()
            elif self.black_bot == "MatthewBot":
                black_bot_move = self.Matthew_AI_move()
            else:
                raise exception('Bot name not recognised')

            self.make_move(black_bot_move)
